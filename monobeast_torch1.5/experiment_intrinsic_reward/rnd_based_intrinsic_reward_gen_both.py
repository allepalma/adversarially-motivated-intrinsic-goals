...
#----------------------------------Rnd addition----------------------------------
        ...
        rnd_novelty = rnd_novelty_tplus1 - rnd_novelty_t
        mask_intrinsic_reward = batch['episode_state_count'][1:] == 1
        clamped_rnd_novelty = torch.clamp(rnd_novelty, min=0, max=None)
        intrinsic_reward_rnd = 0.05 * clamped_rnd_novelty * mask_intrinsic_reward
...
#----------------------------------Rnd addition----------------------------------
        ...
        if not flags.no_generator:
            ...
            if 'frame' in generator_batch.keys():
                ...
                generator_batch['intrinsic_reward_rnd'] = torch.cat((generator_batch['intrinsic_reward_rnd'],
                                                                     rnd_novelty[is_done].float().to(device=flags.device)), 0)
                ...
                generator_batch['intrinsic_reward_rnd'] = torch.cat((generator_batch['intrinsic_reward_rnd'],
                                                                     rnd_novelty[reached].float().to(device=flags.device)), 0)
            else:
                ...
                generator_batch['intrinsic_reward_rnd'] = (rnd_novelty[is_done]).float().to(device=flags.device)
            ...
            if generator_batch['frame'].shape[0] >= flags.generator_batch_size: # Run Gradient step, keep batch residual in batch_aux
                ...
                # Generator Reward
                def distance2(novelty_bonus, reached):
                    """The function implementing the reward system for the agent"""
                    aux = torch.ones(novelty_bonus.shape).to(device=flags.device)*(reached == 1).float() - \
                          torch.ones(novelty_bonus.shape).to(device=flags.device)*(reached == 0).float()
                    aux *= novelty_bonus
                    return aux

                # This is the standard one described by the article
                if flags.generator_loss_form == 'dummy':
                    ...
                    generator_rewards = torch.tensor(distance2(generator_batch['intrinsic_reward_rnd'], generator_batch['reached'])).to(device=flags.device)
