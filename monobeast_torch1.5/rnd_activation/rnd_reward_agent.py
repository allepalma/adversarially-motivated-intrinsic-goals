...
# For scheduling
parser.add_argument('--rnd_reward_scale_start', default=0.1, type=float,
                    help='The starting point of the intrinsic reward scale')
parser.add_argument('--rnd_reward_scale_end', default=0.01, type=float,
                    help='The ending point of the intrinsic reward scale')
...
def learn(
    actor_model, model, actor_generator_model, generator_model, random_target_network,
        predictor_network, batch, initial_agent_state, optimizer, generator_model_optimizer,
        predictor_optimizer, scheduler, generator_scheduler, flags, max_steps=100.0, lock=threading.Lock()):
    ...
    with lock:
        ...
        #intrinsic_reward_rnd = flags.rnd_reward_scale_start*clamped_rnd_novelty*mask_intrinsic_reward
        #intrinsic_reward_rnd = 0.1*clamped_rnd_novelty
        intrinsic_reward_rnd = 0.1*clamped_rnd_novelty*mask_intrinsic_reward
        ...
        #intrinsic_rewards = intrinsic_rewards*(intrinsic_rewards - 0.9 * (batch["episode_step"][1:].float()/max_steps))+intrinsic_reward_rnd
        intrinsic_rewards = intrinsic_reward_rnd
...
def create_buffers(obs_shape, num_actions, flags, width, height, logits_size) -> Buffers:
    ...
    specs = dict(
        ...
        train_state_count=dict(size=(T + 1,), dtype=torch.float32),  # Add state count dicts
    )
...
def train(flags):  
    ...
    # Set an offset for the intrinsic reward's LR scheduling
    offset = (flags.rnd_reward_scale_start - flags.rnd_reward_scale_end)/flags.total_frames 
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            stats = learn(model, learner_model, generator_model, learner_generator_model,
                          ...
                          agent_state, optimizer, generator_model_optimizer,
                          predictor_optimizer, scheduler,
                          generator_scheduler, flags, env.max_steps)
            
            flags.rnd_reward_scale_start = max(flags.rnd_reward_scale_start - offset*T*B, flags.rnd_reward_scale_end) 
